...
function optionsframework_add_admin() {
   ...
   // $of_page = add_submenu_page('themes.php', THEMENAME, 'Theme Options', 'edit_theme_options', 'optionsframework','optionsframework_options_page'); // Default
	...
	if ( function_exists('add_object_page') ) {
		$of_page = add_object_page( THEMENAME .' Options', THEMENAME, 'manage_options', 'optionsframework', 'optionsframework_options_page', ADMIN_DIR .'images/slicetheme.png' );
	}
	else {
		$of_page = add_menu_page( THEMENAME .' Options', THEMENAME, 'manage_options', 'optionsframework', 'optionsframework_options_page', ADMIN_DIR .'images/slicetheme.png' );
	}
...
function of_style_only(){
	...
	//wp_enqueue_style('admin-style', 'http://192.168.1.36/goode/css/admin-style.css');
...
function of_load_only() {
	...
	wp_enqueue_script('custom', ADMIN_DIR . '/js/custom.js', 'jquery');
	...
	foreach ($options as $value) {
		...
		//Start Heading
		 if ( $value['type'] != "heading" )
		 {
			...
			if ( !empty($value['name']) ) $output .= '<h3 class="heading">'. $value['name'] .'</h3>'."\n";
		...
		case 'text':
			/* SliceTheme */ $data[$value['id']] = ( isset($data[$value['id']]) ) ? $data[$value['id']] :$value['std'];
		...
		case 'select':
			...
			foreach ($value['options'] as $select_ID => $option) {			
				...
				/* SliceTheme */ $output .= '<option id="' . $select_ID . '" value="'.$select_ID.'" ' . selected($data[$value['id']], $select_ID, false) . ' />'.$option.'</option>';	 
			 } 
			$output .= '</select></div>';
		break;
		case 'textarea':	
		...
		case 'typography':
			...
			/* SliceTheme */ $typography_stored['size'] = isset($typography_stored['size']) ? $typography_stored['size'] : $value['std']['size'];
			...
			/* SliceTheme */ $typography_stored['height'] = isset($typography_stored['height']) ? $typography_stored['height'] : $value['std']['height'];
			...
			/* SliceTheme */ $typography_stored['face'] = isset($typography_stored['face']) ? $typography_stored['face'] : $value['std']['face'];
			...
			/* SliceTheme */ $typography_stored['style'] = isset($typography_stored['style']) ? $typography_stored['style'] : $value['std']['style'];
		...
		case 'st_images':
			...
			foreach ($value['options'] as $primary => $secondary) 
			{ 
				$output .= '<div style="clear: both;"><h4>'. $primary .'</h4></div>';
				
				foreach ($secondary as $key => $option) 
				{ 
				$i++;
